...
- Family council (**coded**): after clerk path `Act1MoralUnlock` → `FamilyCouncilPending` → night → 6-screen dinner → `FamilyCouncilDone` + **`FamilyLiberationGateOpen`**. See `docs/economy.md` §0–0c.
- Hall/kitchen **scene** buttons (canon labels): **Защитить** / **Наблюдать** / **Отвернуться** (`protect_hard` / `watch` / `ignore`). No separate profit button; tips: protect=no, watch=yes (eyes on line), ignore=best if no slap. Full trio only after **`FamilyLiberationGateOpen`**; before → protect only. Policy labels: **сдержанно** / **сама** / **расковано** (1/2/3). Scene texts deferred.
- **Play profiles + live-NTR + OffenseDays (design lock):** clean/mixed/dirty canvases; sister hard NTR only **live**; skip spark = soft + retry; no morning «already happened». **OffenseDays** (start 4, −1/day, apologize −1, «I was right» ×1, cheap/−days, expensive=0, no NTR escalate). Dual protect hall+kitchen by stats; bans on/off only. Sandra: daughter offense without GG present; no Draupnir-meddle offense yet. Becky dance neglect only if GG was at dances. No offense: Lizette/Georgette; Legare/mayor/Eddie separate. Full: `docs/design-character-intimacy-arc.md` §§ live-NTR + «Обида-дни».
