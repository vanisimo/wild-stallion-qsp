...
- **Play profiles + live-NTR + OffenseDays (design lock):** clean/mixed/dirty; sister hard NTR only **live**; skip spark = soft + retry. **OffenseDays** (start 4, −1/day, apologize −1, «I was right» ×1, gifts). Dual protect/policy hall+kitchen: **accumulate K=3** (not 1 miss = lock — protects race window); ban = immediate; intercept already cumulative. Sandra daughter without GG present; no Draupnir-meddle. Becky dance neglect only if at dances. Full: `docs/design-character-intimacy-arc.md` §§ live-NTR + «Обида-дни». Machine handoff: `docs/session-handoff-2026-07-14.md`.
