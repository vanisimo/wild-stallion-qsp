...
**Секс ГГ с Лизеттой (intim, не платный «снять»):** гейт **`LizetteSexUnlocked`** — после **G2** (второй акт spy), не на F3 и не на этапе B.

---

#### Этап G — Подсмотр: священник + Лизетта (два акта, две недели)

Подробности UI и spy unlock: `docs/design-interaction-schemes.md` §9.

| Шаг | Условие | Сцена |
|-----|---------|--------|
| G1 | **+1 воскресенье** после F3 | Полдень: spy **Лизетта + священник** — допрос (видела мать+ГГ, хотела бы сама, делала ли); жрец **только обнимает** → `LizettePriestSpyStage = 1` |
| G1b | Ночь, порт | Жоржетта: «дочь расплакалась» |
| G2 | **+1 воскресенье** после G1 | Полдень: spy — продолжение → секс → `LizettePriestSpySeen = 1`, `LizetteSexUnlocked = 1` |
| G2b | Ночь, порт, talk с Жоржеттой | «Девочка созрела, пусть идёт **работать со мной**» → `LizetteProstPort = 1` (legacy `ProstStart`) |
