...
**Статус:** дизайн **согласован**. Код — **TODO** (единый движок + reasons по фазам).

### Движок (один на всех, у кого есть обида)

```
reason → OffenseDays = 4 (единый старт; playtest)
пока > 0: обычный talk закрыт; «нос задран»
...
**Проблема:** 1× «не защитил» = OffenseDays=4 → talk-lock на дни → **затягивает** player-path и **ломает гонку** (home first). Это слишком жёстко для одной ошибки.
