...
## Профили прохождения и live-NTR (канон 2026-07)

**Статус:** дизайн **согласован** (обсуждение). Код — **частично** (path choice, гонки, hall menus); петля «искра / intercept / обида» — **TODO**.

Три пути — **не** кнопки на старте и **не** ending-id, а **канвы** из поведения. Игрок может ошибаться и мискликать; важна **общая канва**, не perfect play.

| Профиль | Канва | Hard NTR сестёр |
|---------|--------|-----------------|
| **Чистый** | Узда: protect / policy сдержанно / pursue или запрет / home first | **Не** должен случаться «сам»; срыв только если игрок **системно** отпустил |
| **Смешанный** | Разное **по девушке** (эту держу, за той смотрю) | Разные счётчики; soft bleed семьи, **не** auto-first второй сестры |
| **Грязный** | Watch / allow / policy расковано / late race | Контент **live**; «увидеть всё» ≠ 100% exclusive-сцен за один прогон |

### Гонка «кто первый»

| | Смысл |
|--|--------|
| Победа ГГ (`*HomeSex = 1`) | Закрывает **NPC-first** (гонка Легаре / музыканта **не** стартует) |
| Не даёт | Вечную броню от **позднего** NTR |
| Поражение | NPC first — **понятная** ошибка темпа, не «рандом срыва» |

### После home first: давление → искра → пресечение

```
давление ↑ (зал, NPC interest, Lizette, params, policy)
    → ИСКРА (всегда visible для игрока)
        → пресёк     → hard = 0; +tension/бунт; intercept++
        → смотрел / отпустил → LIVE-шаг NTR (эскалация в кадре)
        → пропуск окна → NEVER auto-sex; soft only; **повтор искры позже**

intercept ≥ N (playtest, черновик ~3 серьёзных) → обида-дни (см. § ниже)
```

| Правило | Канон |
|---------|--------|
| Hard NTR сестёр (Аманда / Мелисса) | Только **live**: видимый старт + выбор **не** пресечь |
| Пропуск окна искры | **Не** sex; soft; **повтор позже** |
| Утренний «уже было, пока тебя не было» | **Запрещён** (и на dirty allow) |
| Voyeur-regret | Только если **был** на искре / досмотрел |
| Запрет (`Prohibit*`) | Только **да/нет** (без «смягчить запрет»). Потолок **hard**, не zero content: давление и искры **растут** |
| Hidden completed sex сестёр при узде | **Вне канона** |
| Сандра + Драупнир | **Своя жизнь**; ГГ **пока не** лезет в эти отношения; **не** reason обиды «лезешь к гному» |
| Город / семья | Обиды — общий движок (§ ниже); Легаре / мэр / Эдди — **свои** конфликты, не OffenseDays |

### Связь с залом / ужином

- `FamilyLiberationGateOpen` открывает **кнопки** (Наблюдать / Отвернуться, policy расковано), **не** auto-slut.
- До ужина в сценах по сути только **Защитить** + policy сдержанно (см. `docs/economy.md` §0c).
- Чистый **после** ужина может **дальше** только protect + сдержанно.
- Dual protect (обида): **зал и кухня** — одни правила.

### Что не обещаем

- Один прогон dirty = 100% всех exclusive-узлов (гонка first, pursue vs allow, мэр честный vs грязный).
- Perfect mixed без soft bleed `FamilyCorruptionStage` / сплетен.
- «Запретил и забыл» = вечная тишина без искр и без семейной цены.

---

## Обида-дни (канон 2026-07)

**Статус:** дизайн **согласован** (движок + отказ клиентам).  
**Код:** OffenseDays engine / talk — частично; **missing bargain refuse — v1 вшит** (см. `docs/design-hall-missing-bargain-offense.md`); harass-intro refuse — backlog.  
**Связь:** `offense_days*.qsps`, `docs/design-hall-missing-bargain-offense.md`, `docs/design-tavern-client-ambient.md`.

### Движок (один на всех, у кого есть обида)

```
reason → OffenseDays = 4 (единый старт; playtest)
пока > 0:
  обычный talk закрыт; «нос задран»
  intimate с ГГ закрыт (уже в текстах)
  ★ клиентская грязь: отказ (см. § «Обида → отказ клиентам»)
  −1 / сутки (обида может пройти сама)
  talk: только сцена обиды
    → она выговаривается (по reason)
    → Извиниться (−1 день)
    → «Я всё сделал правильно» (×1 за обиду; реплика; дни не меняются; без эскалации)
    → Подарок если есть
  gift_cheap: −дни +stats, но бонус **меньше**, чем gift без обиды
  gift_expensive: OffenseDays = 0 («как раз…») +stats **меньше**, чем без обиды
→ 0: soft clear («братик / сынок… как я могу злиться»)
```

| Правило | Канон |
|---------|--------|
| Эскалация (NTR / «ушла в тень» за упрямство) | **Нет** |
| Запрет | только **вкл/выкл**; не из меню обиды «смягчить» |
| «Я прав» | **один раз** за текущую обиду |
| Извинение | **−1** день за раз (можно повторять) |
| Старт дней | **4** для всех (пока) |
| Одна сцена → | не стакать 3 полных обиды; dual protect — **накопление**, не мгновенный lock |
| Hall harassment **protect** как работа | **не** intercept-счётчик; dual protect — **отдельные** mismatch-счётчики по статам |
| Пока `GirlOffenseDays > 0` | talk/intimate с ГГ закрыты; **missing-уговор:** клиент говорит → отказ (кроме **нобиля**); harass refuse — backlog |

### Обида → клиенты / уговор (канон 2026-08)

**Суть:** зла на Стефана → **шлёт** обычных клиентов (укол в хозяина), не «я скромная».

#### Missing bargain — **сделано** (править prose)

Полная схема, promise-id, нобиль, пьяный, файлы:
...
| Случай | Поведение |
|--------|-----------|
| Обычный `$p` + обида | клиент **как в ветке** → ответ-**отказ** (`OffenseAnswerText` по id) → no missing |
| `noble` + обида | **не** отказ; язык (Аманда) / довольный взгляд (Мелисса); гордо ушла, **без** разговора с ГГ |
| `drunk_guest` | **всегда** отказ |
| Harass / kitchen (не bargain) | **backlog** — force resist + укол |

#### Harass (backlog)

```
клиент лезет (harass)
  → if GirlOffenseDays > 0:
       resist; короткий отказ + укол в ГГ
       НЕТ play → missing
  → else: v2 flow
```
...
- NTR из обиды («ушла к клиенту назло») — **запрет**, кроме осознанного **нобиль+обида** (она *идёт* с нобилем, укол хозяину — не «эскалация NTR-движка»).
- Полный ban harass (тишина зала).
- Отказ после clear offense.
- Выдуманные promise id в отказах (нет `titjob` у Аманды).

### Dual protect / policy — **накопление** (зал **и** кухня)

**Проблема:** 1× «не защитил» = OffenseDays=4 → talk-lock + **отказ клиентам** на дни → **затягивает** player-path и **ломает гонку** (home first). Это слишком жёстко для одной ошибки — поэтому mismatch **копится** (K≈3), а не мгновенный lock.

**Канон:** mismatch **копится**; полная обида — только с **порога**.

```
статы низкие  →  ждут защиты и сдержанных правил
                 не защитил / «расковано»  →  MismatchCount++

статы высокие →  ждут свободы и доверия
                 защитил «как ребёнка» / только «сдержанно»  →  MismatchCount++

середина     →  без счётчика (явные low/high band)

if MismatchCount >= K (K = 3, playtest):
    OffenseDays = 4
    MismatchCount → 0 или 1 («память»)
else:
    talk НЕ закрыт; soft: −trust / +tension / короткая реплика «опять…»
```

| Параметр | Значение (playtest) |
|----------|---------------------|
| **K** (порог dual protect + policy mismatch) | **3** |
| Сброс count | **7 дней** без mismatch → count −1 или reset |
| Пока count &lt; K | **нет** OffenseDays / talk-lock |
| После порога | обычный движок обиды-дней |

То же накопление для **`policy_mismatch`** (назначение правила «в штыки»): +1 к счётчику / отдельный policy-count с K=2–3 — не мгновенный lock с первого клика.

### Сразу vs накопление (сводка)

| Reason | Режим |
|--------|--------|
| `no_protect_low`, `protect_high`, `policy_mismatch` | **Накопление** (K≈3); 1 раз ≠ полная обида |
| `intercept` | уже **накопление** (≥N~3 серьёзных пресечений) |
| `ban_*` | **сразу** OffenseDays (явный запрет) |
| `daughter_no_protect` (Сандра) | **сразу** или K=2 (мама реже; по умолчанию **сразу**) |
| neglect Бекки/Инга | накопление **временем** (долго не…) |
| `dance_boundary`, `gg_harass` | v2/v3; 2× или сразу — при коде |

### Кто в системе / кто нет

| OffenseDays | Вне системы |
|-------------|-------------|
| Аманда, Мелисса | **Лизетта, Жоржетта** — без обид |
| Сандра | **Легаре, мэр, Эдди** — свои конфликты |
| Бекки (после старта отношений) | |
| Инга (после старта / лестница) | |
| Кларисса (ревность / post-affair) | |
...
| Reason | Когда | Режим |
|--------|--------|--------|
| `no_protect_low` | Не защитил (зал/кухня), низкие статы | **накопление** K=3 |
| `protect_high` | Защитил при высоких статах | **накопление** K=3 |
| `policy_mismatch` | Правило зала/кухни «в штыки» | **накопление** |
| `intercept` | Пресёк серьёзную искру | **накопление** ≥N~3 |
| `ban_guys` / `ban_minstrel` / `ban_lizette` | Включил запрет (да/нет) | **сразу** |
| `dance_boundary` | На танцах пересёк дозволенное (**v2**) | TBD |
| `gg_harass` | Приставания ГГ не в тему (**v3**) | TBD |

Запрет **Лизетты** → обида **Аманды** (`ban_lizette`), не обида Лизетты.

#### Сандра

| Reason | Когда |
|--------|--------|
| `daughter_no_protect` | Не защитил дочь; мама **узнала** (плач/рассказ) — **достаточно без присутствия ГГ** |
| `policy_mom` / порядок про девочек и зал | Правила «в штыки» с точки зрения матери |
| Драупнир / «лезешь» | **Нет (пока)** — ГГ не лезет в эти отношения |

#### Бекки (после начала отношений)

| Reason | Когда |
|--------|--------|
| `neglect_visit` | Долго не приходил |
| `neglect_sex` | Долго не было близости (уже была) |
| `neglect_dance` | **Был** на танцах и **не** потанцевал с ней; если ГГ не был на танцах — **не** этот reason |

#### Инга

| Reason | Когда |
|--------|--------|
| `neglect_*` | Как Бекки (визит / танец / intim после старта) |
| `rush_ladder` | Прыжок по лестнице раньше порогов |

#### Кларисса

| Reason | Когда |
|--------|--------|
| `jealous_amanda_dance` | Ушёл с танцев с Амандой (зона ревности) |
| `post_affair_rel` | После измены мужа и романа с ГГ — свои triggers (**v2+**) |

### Фазы внедрения кода

| Фаза | Содержание |
|------|------------|
| **v1** | Движок OffenseDays + меню; dual protect/policy **накопление K=3**; intercept; ban **сразу**; кухня; Сандра `daughter_no_protect` |
| **v2** | `dance_boundary`; Бекки/Инга neglect; Инга rush; Кларисса jealous dance |
| **v3** | `gg_harass`; расширенная Кларисса post-affair |

### Audit кода vs live-NTR (2026-07, снимок)

| Узел | Live spark + menu? | Hard без live / без знания? | Вердикт |
|------|--------------------|-----------------------------|---------|
| `AmandaLegareFridayRace` stop/claim | да | hard нет | ✅ |
| same **watch** + roll → `AmandaLegareFirstSexDone` | spark да; sex часто **флаг+текст** | player **выбрал** смотреть | ⚠️ явный live-beat |
| same **step_aside** + roll → FirstSex, **`PlayerKnows=0`** | spark да | **hard без знания** | ❌ против канона |
| `MelissaMinstrelFridayRace` / **Backroom** leave+roll | spark да | FirstSex + leave | ❌/`⚠️` |
| `HallMissingGirl` ignore | spark да | PrivateCount/NpcPath, **не** FirstSex | ⚠️ soft ok |
| `HallMissing` peek/interrupt | live | — | ✅ |
| `AmandaLegareAnal` leave | leave **без** Done | — | ✅ |
| `AmandaLizetteSecretVisit` | visible + choice | не sex-hard | ✅ |
| `SandraDraupnir*BackgroundResolve` | нет | AffairStage++ без ГГ | ✅ мама, не обида-движок |
| OffenseDays / dual protect | — | — | ❌ **нет в коде** |
| Очередь искры при AFK | — | — | ❌ **нет в коде** |

**Приоритет кода:**  
1) `step_aside` / `leave` — never FirstSex без live; soft + retry.  
2) watch hard — только явный live.  
3) OffenseDays v1 (сёстры + Сандра-за-дочь + dual protect/policy).
...
*Последнее обновление: § live-NTR + §«Обида-дни» (2026-07); §Sex kinks; PR-I0…I16; таймлайн **28/35**, гонки **33/40**.*