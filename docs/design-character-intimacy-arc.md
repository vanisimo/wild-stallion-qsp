...
| Стакать reasons | одна сцена → одна обида; кулдаун на повтор того же reason |
| Hall harassment **protect** как работа | **не** intercept-счётчик; dual protect — **отдельные** reasons по статам |

### Dual protect / policy (зал **и** кухня)

```
статы низкие  →  ждут защиты и сдержанных правил
                 не защитил / «расковано»  →  обида

статы высокие →  ждут свободы и доверия
                 защитил «как ребёнка» / только «сдержанно»  →  обида

середина     →  без авто-обиды (явные low/high band)
```
...
| Reason | Когда |
|--------|--------|
| `no_protect_low` | Не защитил (зал/кухня), низкие статы |
| `protect_high` | Защитил при высоких статах |
| `policy_mismatch` | Правило зала/кухни «в штыки» относительно статов |
| `intercept` | Пресёк серьёзную искру (счётчик ≥N ~3) |
| `ban_guys` / `ban_minstrel` / `ban_lizette` | Включил запрет (да/нет) |
| `dance_boundary` | На танцах пересёк дозволенное (**v2**) |
| `gg_harass` | Приставания ГГ не в тему (**v3**) |
...
| Фаза | Содержание |
|------|------------|
| **v1** | Движок OffenseDays + меню; сёстры dual protect/policy + intercept + ban; кухня; Сандра `daughter_no_protect` / policy_mom |
